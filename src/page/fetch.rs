//! # 数据获取页面模块
//!
//! 提供交易数据获取的用户界面，支持多种时间范围选择和自定义日期输入。
//! 实现了异步数据获取、进度显示和状态管理功能。
//!
//! ## 核心功能
//!
//! - **时间范围选择**: 预设的时间段选项（1年、3个月、1个月）
//! - **自定义日期**: 用户可输入特定的开始日期
//! - **进度显示**: 实时显示数据获取进度和状态
//! - **本地缓存**: 显示本地数据库中的交易记录数量
//! - **异步获取**: 后台异步获取数据，不阻塞 UI 操作
//!
//! ## 页面布局
//!
//! ```text
//! ┌─────────────────────────────────────────────────────────┐
//! │ [Past 1 year] [Past 3 months] [Past 1 month] (选择按钮) │
//! ├─────────────────────────────────────────────────────────┤
//! │ Custom Start Date: [用户输入框]                         │
//! ├─────────────────────────────────────────────────────────┤
//! │                                                         │
//! │ Local DB Count: 1234 transactions                      │
//! │ Status: [Idle | Fetching | Complete]                   │
//! │ Progress: Page 5/10, 500 transactions fetched          │
//! │                                                         │
//! ├─────────────────────────────────────────────────────────┤
//! │ 快捷键帮助信息                                          │
//! └─────────────────────────────────────────────────────────┘
...
//!     current_page: u32,             // 当前页数
//!     total_entries_fetched: u32,    // 已获取条目数
//!     oldest_date: Option<DateTime>, // 最早交易日期
//! }
//! ```
//!
//! ## 异步通信
...
//!
//! ```rust
//! use crate::page::fetch::Fetch;
//! use crate::libs::fetcher::MealFetcher;
//!
//! // 创建获取页面
//! let mut fetch_page = Fetch::new(action_sender, transaction_manager)
//!     .client(MealFetcher::Real(real_fetcher));
//!
//! // 页面会处理所有用户交互和数据获取逻辑
//! ```
